    ...
    const questions = [
      {
        ...
        message: chalk.magenta(
          `Signing uptime vote for reward epoch: ${rewardEpochId} and Merkle root: ${hash} (default hash of zero). \nDo you want to proceed?`
        ),
        choices: ["Yes", "No"],
        filter: (val: string) => {
    ...
    const questions = [
      {
        ...
        message: chalk.magenta(
          `Signing rewards for reward epoch: ${rewardEpochId}, Merkle root: ${hash} and number of weight based claims: ${noOfWeightBasedClaims}. \nDo you want to proceed?`
        ),
        choices: ["Yes", "No"],
        filter: (val: string) => {
          return val == "Yes" ? true : false;
